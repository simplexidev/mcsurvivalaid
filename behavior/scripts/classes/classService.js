...
export async function maybePromptClassChange(player, worldDay) {
  ...
  if (confirm.canceled || confirm.selection !== 0) {
    state.classTrack.nextClassChangePromptDay = worldDay + 10; setPlayerState(player, state); return;
  }
  const available = getClassList().filter(c => c.id !== state.classTrack.currentClass && !state.classTrack.completedClasses.includes(c.id));
  if (available.length === 0) { player.sendMessage("All classes completed or unavailable."); state.classTrack.nextClassChangePromptDay = worldDay + 10; setPlayerState(player,state); return; }
  ...
  if (result.canceled || result.selection === undefined) { state.classTrack.nextClassChangePromptDay = worldDay + 10; setPlayerState(player, state); return; }
