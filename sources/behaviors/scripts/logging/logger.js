...
function getDynamicProperty(name, fallback) {
    try {
        const value = server_1.world.getDynamicProperty(name);
...
function setDynamicProperty(name, value) {
    try {
        server_1.world.setDynamicProperty(name, value);
        return true;
    }
    catch (error) {
        warnDynamicPropertiesUnavailable(error);
...
function warnDynamicPropertiesUnavailable(error) {
    if (warnedDynamicPropertiesUnavailable)
        return;
    warnedDynamicPropertiesUnavailable = true;
    console.warn(formatLine("warn", "logger", "Dynamic properties unavailable yet; using cached/default logger configuration.", { error: String(error) }));
}
...
function shouldEmitTrace(scope, message) {
    const tick = Number(server_1.world.getAbsoluteTime?.() ?? 0);
...
function emit(level, scope, message, context) {
    if (!shouldLog(level))
        return;
    if (level === "trace" && !shouldEmitTrace(scope, message))
        return;
