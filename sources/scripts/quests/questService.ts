...
function questKey(category: QuestCategory, key: string, tier: number): string {
  return `${category}:${key}:${tier}`;
}
function isPlayer(entity: Entity | Player): entity is Player {
...
export function addQuestProgress(
  player: Entity | Player,
  category: "blocksBroken" | "blocksPlaced",
  key: BlockMetricKey,
  amount: number
): void;
export function addQuestProgress(player: Entity | Player, category: QuestCategory, key: string, amount: number): void {
  ...
  thresholds.forEach((threshold, idx) => {
    ...
    const already =
      state.quests.claimedQuestRewards.includes(token) ||
      state.quests.pendingQuestRewards.some((q) => q.token === token);
