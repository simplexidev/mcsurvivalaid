...
export type TravelMetricKey = "horizontal_distance" | "swim_distance" | "jump_count" | "fall_distance" | "boat_distance" | "glide_distance";
export type BlockMetricKey = "ground" | "ore" | "fauna" | "decorative";
export type CombatMetricKey = "hostile_mobs_killed" | "non_hostile_mobs_killed" | "damage_taken" | "damage_dealt" | "gear_crafted" | "gear_smelted" | "gear_broken";
...
export interface ItemRequest { claimed: boolean; itemId: string; quantity: number; readyAtTick: number; requestedAtTick?: number; notifiedReady?: boolean; etaTicks?: number }
export interface SavedTeleportPoint { dimension: string | Dimension; x: number; y: number; z: number; tick?: number; day?: number }

export interface ActionFormResult { canceled: boolean; selection?: number; formValues?: (boolean | number | string)[] }
export interface ModalFormResult { canceled: boolean; formValues?: (boolean | number | string)[]; selection?: number }
export interface SafeFormError { error?: string }
...
export interface PlayerState {
  ...
  settings: {
    showHud: boolean; showDaysSurvived: boolean; showDaysUntilReward: boolean; showRewardReady: boolean;
    chestChangesTexture: boolean; allowTeleportToDeath: boolean; allowTeleportToRespawn: boolean; teleportCooldownSeconds: number;
  };
}
...
export type QuestMetricKey<C extends QuestCategory> =
  C extends "travel" ? TravelMetricKey :
  C extends "combat" ? CombatMetricKey :
  BlockMetricKey;
