...
const PAGES = [
  ...
  [
    "Survival Chest",
    "Claim pending class rewards, quest rewards, and completed item requests from your registered chest.",
  ],
  ["Book of Survival", "Main UI entry: structure locator, item requests, teleports, docs, settings."],
  [
    "Classes & Rewards",
    "Choose Adventurer/Warrior/Miner/Mage. Earn tier rewards at day milestones and recurring rewards later.",
  ],
...
export async function showDocumentationMenu(player) {
  ...
  if (res.selection === PAGES.length) {
    ...
    await safeShow(
      new ActionFormData().title("Quest Progress Snapshot").body(body).button("Back"),
      player,
      "documentationMenu",
      "quest_snapshot"
    );
