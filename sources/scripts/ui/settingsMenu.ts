...
export async function showSettingsMenu(player) {
  const root = new ActionFormData()
    ...
    .button("Gameplay Settings")
    .button("Recover Starter Items")
    .button(`Content Log Level: ${logger.getLogLevel()}`)
  ...
  if (rootResult.canceled || rootResult.selection === undefined) {
    logger.trace("settingsMenu", "Settings root canceled", { playerId: player.id });
    return;
  }
  ...
  if (result.canceled || !result.formValues) {
    logger.trace("settingsMenu", "Gameplay settings canceled", { playerId: player.id });
    return;
  }
  ...
  const teleportCooldownSeconds = Math.floor(
    getNumberField(result.formValues, 7, state.settings.teleportCooldownSeconds)
  );
...
async function showLoggingLevelMenu(player) {
  ...
  const form = new ActionFormData()
    .title("Content Log Level")
    .body("Choose how verbose the Survival Aid content log should be.");
