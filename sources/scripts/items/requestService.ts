...
const REQUESTABLE_ITEMS: ReadonlyArray<string> = [
  "coal",
  "raw_iron",
  "raw_copper",
  "raw_gold",
...
export async function showItemRequestsMenu(player: Player): Promise<void> {
  const root = new ActionFormData()
    .title("Item Requests")
    .button("Create Request")
    .button("View Active Requests")
    .button("Cancel Request");
  const rootResult = await safeShow(root, player, "requestService", "root");
  if (rootResult.canceled || rootResult.selection === undefined) {
    logger.trace("requestService", "Request root menu canceled", { playerId: player.id });
    return;
  }
  ...
  if (result.canceled || result.selection === undefined) {
    logger.trace("requestService", "Request item selection canceled", { playerId: player.id });
    return;
  }
  ...
  if (qtyResult.canceled || !qtyResult.formValues) {
    logger.trace("requestService", "Request quantity canceled", { playerId: player.id });
    return;
  }
...
function showActiveRequests(player: Player): void {
  ...
  if (active.length === 0) {
    logger.trace("requestService", "No active requests to display", { playerId: player.id });
    return player.sendMessage("No active item requests.");
  }
  const lines = active.map(
    (r, i) =>
      `${i + 1}. ${r.itemId} x${r.quantity} (${Math.max(0, Math.ceil((r.readyAtTick - system.currentTick) / 20))}s)`
  );
...
async function showCancelRequestMenu(player: Player): Promise<void> {
  ...
  const active = state.requests.active.filter((r) => !r.claimed);
  if (active.length === 0) {
    logger.trace("requestService", "No active requests to cancel", { playerId: player.id });
  ...
  logger.info("requestService", "Request canceled", {
    playerId: player.id,
...
export function queueItemRequest(player: Player, itemId: string, quantity: number): void {
  ...
  state.requests.active.push({
    itemId,
    quantity,
    requestedAtTick: system.currentTick,
    readyAtTick: system.currentTick + duration,
    claimed: false,
  });
  setPlayerState(player, state);
...
export function collectReadyItemRequests(state: PlayerState): ItemRequest[] {
  ...
  for (const req of state.requests.active) {
    if (!req.claimed && req.readyAtTick <= system.currentTick) {
      ready.push(req);
      logger.trace("requestService", "Request ready for claim", {
        itemId: req.itemId,
        quantity: req.quantity,
        readyAtTick: req.readyAtTick,
      });
      req.claimed = true;
    }
  }
